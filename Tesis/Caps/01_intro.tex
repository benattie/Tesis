...
\begin{equation}
 ...
 \end{equation}
\noindent
En este caso, si se supone que las cristalitas son de forma esférica y que su distribución de tamaño de de tipo lognormal y se supone un perfil de deformación de tipo gaussiano, se puede llegar a la ecuación que se emplea cuando se aplica el método de Warren-Averbach:
\begin{equation}
...
Finalmente la causa (v) podrá visualizarse en ciertos casos estudiando la asimetría de los picos de difracción, así como estudiando la dependendencia de la anisotropía con el orden de difracción {hkl}.
...
A partir de este ejemplo es que se puede pensar en el factor de contraste $C$ como una cantidad que va a modificar la visibilidad de las dislocaciones en un experimento de difracción.
...
El cálculo de C a partir de la Ec \ref{eq:contrast} para un material elásticamente anisotropíco es una tarea que además de laboriosa, requiere conocer a priori del tipo de dislocaciones presentes en el material y los sistemas de deslizamiento activos, lo que en general no ocurre.
Sin embargo, Ungar et al han deducido expresiones analíticas para calcular el factor de contraste promedio para materiales de simetría cristalina cúbica y hexagonal. Para materiales cúbicos la Ec. \ref{eq:contrast} toma una forma particularmente simple:
...
\nomenclature{$H^2$}{Invariante de cuarto orden que determinal el factor de contraste promedio.}
\nomenclature{$\bar{C}_{h00}$}{Factor de contraste promedio para los planos {h00}}
\nomenclature{$q$}{Caracter hélice-borde de una dislocación}
...
Suposiciones hechas para sostener el concepto de factores de contraste.
