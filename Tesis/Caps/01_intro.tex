\chapter{Introducción}\label{C:Intro}
...
\nomenclature{$H$ o $FWHM$}{Ancho de pico a media altura. También abreviado como FWHM por sus siglas en inglés (Full Width at Half Maximum).}
\nomenclature{$\eta$}{Factor de mezcla de una función pseudo-Voigt, o bien componente Lorentziana de la misma.}
...
En los materiales FCC puede demostrarse que el ensanchamiento puede observarse solamente en los planos que cumplen $h \ + k \ + \ l \ = \ 3n, \ n \in \mathbb N$.
...
\nomenclature{EBSD}{Difracción de electrones retrodifundidos}
...
Para resolver el problema de inversión de las figuras de polos, es decir, para obtener $f(\mathbf{g})$ a partir de la medición de diferentes $p_{\mathbf{h}}(\mathbf{y})$, Bunge partió del hecho que $f(\mathbf{g})$ puede desarrollarse en armónicos esféricos:
\begin{equation}
